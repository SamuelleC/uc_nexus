...
function generateFloors(totalFloors = 10, roomsPerFloor = 10): Floor[] {
	const floors: Floor[] = [];
	let nextRoomId = 1;
	...
	for (let f = 1; f <= totalFloors; f++) {
		...
		for (let r = 1; r <= roomsPerFloor; r++) {
			...
			rooms.push({ id: nextRoomId++, name: `U${roomNumber}`, occupied: (nextRoomId % 3) === 0 });
		}
...
export default function UBuilding() {
	const [floors, setFloors] = useState<Floor[]>(() => generateFloors(10, 10));

	return (
		<SafeAreaView className="flex-1 bg-gray-100">
			<Header title="U Building" subtitle="Rooms and availability" />
							...
							{floor.rooms.map((room) => (
								<View key={room.id} className="flex-row items-center px-4 py-3 border-t border-gray-100">
									...
									<View className="w-32 flex-row justify-end">
										<View className={
											'px-3 py-1 rounded-full ' + (room.occupied ? 'bg-red-600' : 'bg-green-600')
										}>
